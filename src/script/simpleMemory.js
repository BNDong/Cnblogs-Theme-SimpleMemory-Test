...
function simpleMemory() {
    ...
    this.baseStyleCheck = () => {
        ...
        // check base theme
        let baseStyle = mobileStyle.attr('href');
        if (typeof baseStyle != 'undefined') {
            let bt = baseStyle.split('/');
    ...
    this.initVersion = () => {
        ...
        function setConfVersion() {
            let confVersion = getEndConfVal(window.cnblogsConfig.GhVersions);
    ...
    this.initTheme = () => {
        ...
        let url  = window.location.href,
            tmp  = url.split("/"),
            user = tmp[3],
            navListHtml = '<li><a href="https://www.cnblogs.com/'+user+'/" target="_self"><i class="iconfont icon-homepage_fill"></i>首页</a></li>' +
        ...
        let menuNavList = window.cnblogsConfig.menuNavList;
        ...
        // load loadingJs
        $.getScript(__sm.getJsDelivrUrl('loading.js'), function () {
            ...
            $.getScript(__sm.getJsDelivrUrl('lib/jquery.mCustomScrollbar.min.js'), function () {
                $.getScript(__sm.getJsDelivrUrl('lib/require.min.js'), function () {
                    $.getScript(__sm.getJsDelivrUrl('config.js'), function () {
                        let staticResource = [
     ...
     * @returns {string}
     */
    this.setFileNameMin = (file, directory) => {
        if (typeof file === 'undefined') return '';

        let suffix = null,fileArr = file.split('.');
